...
import os
##
# sys.path.append("C:\Users\Jung\PycharmProjects\AutoStock")
...
class Kiwoom(QAxWidget):
    ...
    def trdata_slot(self, sScrNo, sRQName, sTrCode, sRecordName, sPrevNext):
        ...
        if sRQName == "예수금상세현황요청":
            ...
            print("예수금 : %s 원 " % format((self.deposit),","))
            print("출금가능금액 : %s 원 " % format((self.output_deposit), ","))
            ...
            print("(계좌평가잔고내역요청 싱글데이터) 총매입 : %s 원 || 총손익 : %s 원 || 수익률 : %s  " % (int(total_buy_money), int(total_profit_loss_money), float(total_profit_loss_rate)))
            ...
            for i in range(rows):
                ...
                print("종목코드: %s // 종목명: %s // 보유수량: %s // 매입가:%s // 수익률: %s // 현재가: %s" % (
                    code, code_nm.rstrip(), int(stock_quantity), int(buy_price), float(learn_rate), int(current_price)))
            ...
            if sPrevNext == "2":
                ...
                if pass_success == True:
                    ...
                    f = open("files/condition_stock.txt", "a", encoding="utf8")   ##open 은 파일을 저장  폴더위치와 저장명/쓰기옵션/인코딩방식
